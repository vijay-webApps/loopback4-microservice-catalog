import {injectable, /* inject, */ BindingScope, Provider, inject} from '@loopback/core';
...
export class AwsTranslateProvider implements Provider<LanguageTranslateProvider> {
  ...
  value() {
    return {
        ...
        async detectDominantLanguage(text: string): Promise<string> {
          const { Languages } = await this.languageDetectorClient.detectDominantLanguage({
            Text: text
          }).promise();
        ...
        async translateText(Text: string, SourceLanguageCode: string, TargetLanguageCode: string): Promise<string> {
          const { TranslatedText } = await this.translateClient.translateText({
            Text,
            SourceLanguageCode,
            TargetLanguageCode,
          }).promise();
